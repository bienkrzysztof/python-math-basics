...
def test_analyze_signals_trade_count():
    signals = pd.DataFrame({
        "signal": [
            "crossover",
            "crossover",
...
def test_analyze_signals_best_and_worst_return():
    signals = pd.DataFrame({
        "signal": [
            "crossover",
            "crossover",
    ...
    assert result["crossover_best_return"] == pytest.approx(0.10)
    assert result["crossover_worst_return"] == pytest.approx(-0.04)
...
def test_analyze_signals_average_return():
    signals = pd.DataFrame({
        "signal": [
            "crossover",
            "crossover",
    ...
    assert result["crossover_average_return"] == pytest.approx(
        (0.10 + 0.05 - 0.04) / 3
    )
